  ...
  <div>
    ...
    <el-row>
      <el-table :data="tableData" border style="width: 100%">
        <el-table-column prop="createByUser.username" label="发布人"/>
  ...
  methods: {
    clickButton(type, row) {
      if (type === 'add') {
        ...
        this.$http.delete(`/yw/news-management/deleteById/${row.id}`)
            .then(() => {
              this.initTableData()
            })
      }
    ...
    currentChange(number) {
      ...
      this.$http.post("/yw/news-management/page", this.page)
          .then(resp => {
    ...
    formSubmit() {
      ...
      if (this.dialog.isAdd) {
        console.log(123123)
        this.$http.post('/yw/news-management/add', this.form)
            .then(() => {
              this.initTableData();
            })
      } else if (!this.dialog.isAdd) {
        this.$http.post('/yw/news-management/update', this.form)
            .then(() => {
              this.initTableData();
            })
      }
