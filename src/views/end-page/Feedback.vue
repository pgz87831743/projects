  ...
  methods: {
    ...
    clickButton(type, row) {
      ...
      if (type === 'add') {
        ...
        feedbackApi.getById(row.id).then((resp) => {
          this.dialog.dialogFormVisible = true
        ...
        feedbackApi.deleteById(row.id).then(() => {
          this.initTableData()
        })
      }
    },
    ...
    currentChange(number) {
      ...
      feedbackApi.page(this.page).then(resp => {
        this.tableData = resp.data.data.records
        this.total = resp.data.data.total
      })
    },
    ...
    formSubmit() {
      ...
      if (this.dialog.optionValue === 'add') {
        feedbackApi.add(this.form)
            .then(() => {
              this.initTableData();
            })
      } else if (this.dialog.optionValue === 'update') {
        feedbackApi.updateById(this.form)
            .then(() => {
              this.initTableData();
            })
      }
