    ...
    <el-dialog v-model="dialog.dialogFormVisible" :title="dialog.optionName" @closed="dialogClose">
      <el-form :model="form" label-position="right" label-width="150px" :disabled="dialog.formDisabled">
        <el-form-item label="车位ID">
          <el-input v-model="form.parkingPlaceId" placeholder="请输入"/>
        </el-form-item>
        ...
        <el-form-item label="出租时间">
          <el-input v-model="form.hireTime" placeholder="请输入"/>
        </el-form-item>
  ...
  methods: {
    ...
    clickButton(type, row) {
      ...
      if (type === 'add') {
        ...
        historyApi.getById(row.id).then((resp) => {
          this.dialog.dialogFormVisible = true
        ...
        historyApi.deleteById(row.id).then(() => {
          this.initTableData()
        })
      }
    },
    ...
    currentChange(number) {
      ...
      historyApi.page(this.page).then(resp => {
        this.tableData = resp.data.data.records
        this.total = resp.data.data.total
      })
    },
    ...
    formSubmit() {
      ...
      if (this.dialog.optionValue === 'add') {
        historyApi.add(this.form)
            .then(() => {
              this.initTableData();
            })
      } else if (this.dialog.optionValue === 'update') {
        historyApi.updateById(this.form)
            .then(() => {
              this.initTableData();
            })
      }
