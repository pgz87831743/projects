      ...
      <el-col :span="1">
        <el-button type="primary" @click="clickButton('add')">新增</el-button>
            ...
            <el-col :span="5" :offset="1">
              <el-input v-model="page.search" placeholder="请输入药店名" clearable/>
            </el-col>
  ...
  methods: {
    ...
    clickButton(type, row) {
      ...
      if (type === 'add') {
        ...
        drugstoreApi.getById(row.id).then((resp) => {
          this.dialog.dialogFormVisible = true
        ...
        drugstoreApi.deleteById(row.id).then(() => {
          this.initTableData()
        })
      }
    },
    ...
    currentChange(number) {
      ...
      drugstoreApi.page(this.page).then(resp => {
        this.tableData = resp.data.data.records
        this.total = resp.data.data.total
      })
    },
    ...
    formSubmit() {
      ...
      if (this.dialog.optionValue === 'add') {
        drugstoreApi.add(this.form)
            .then(() => {
              this.initTableData();
            })
      } else if (this.dialog.optionValue === 'update') {
        drugstoreApi.updateById(this.form)
            .then(() => {
              this.initTableData();
            })
      }
