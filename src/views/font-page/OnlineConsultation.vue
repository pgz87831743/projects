      ...
      <el-col :span="12">
        <el-card>
          <el-tabs @tab-change="tableChange">
            <el-tab-pane label="医生列表" v-if="getUser().role!=='DOCTOR'">
              <el-collapse>
                <el-collapse-item :title="item.nickname" :name="item.username" v-for="item in doctorList"
                                  v-bind:key="item.id">
                  <el-descriptions
                      class="margin-top"
                      title="医生信息"
                  ...
                  >
                    <template #extra>
                      <el-button type="success" @click="onlineTalkHandle(item)">创建会话</el-button>
                    </template>
                    <el-descriptions-item label="医生工号">{{ item.num }}</el-descriptions-item>
                    ...
                    <el-descriptions-item label="擅长领域">
                      <el-input type="textarea" v-model="item.description" :autosize="{minRows:5}" disabled></el-input>
...
<script>
import {messageApi, onlineInfoApi, sysUserApi} from "@/api/api";
