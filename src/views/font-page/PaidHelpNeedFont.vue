      ...
      <div class="p-div">
        <el-row>
          <el-table :data="tableData" border style="width: 100%">
            <el-table-column prop="name" label="求助名称"/>
            <el-table-column prop="description" label="求助说明"/>
            <el-table-column prop="price" label="求助价格"/>
            ...
            <el-table-column label="操作" width="300px">
              <template #default="scope">
                ...
                <el-button
                    ...
                    size="small"
                    type="danger"
                    @click="clickButton('delete',scope.row)">已解决
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-dialog v-model="dialog.dialogFormVisible" :title="dialog.optionName" @closed="dialogClose">
          <div style=" overflow:auto;height:480px;" id="divscope2">
            <div class="lt">
              <div>
...
import {messageApi, paidHelpApi} from "@/api/api";
...
export default {
  ...
  methods: {
    ...
    clickButton(type, row) {
      ...
      if (type === 'delete') {
        paidHelpApi.deleteById(row.id).then(() => {
          this.initTableData()
        })
      }
    ...
    currentChange(number) {
      ...
      paidHelpApi.page(this.page).then(resp => {
        this.tableData = resp.data.data.records
        this.total = resp.data.data.total
      })
    },
    ...
    formSubmit() {
      ...
      if (this.dialog.optionValue === 'add') {
        paidHelpApi.add(this.form)
            .then(() => {
              this.initTableData();
            })
      } else if (this.dialog.optionValue === 'update') {
        paidHelpApi.updateById(this.form)
            .then(() => {
              this.initTableData();
            })
      }
    ...
    scope(){
      let div= document.getElementById("divscope2")
  ...
  mounted() {
    this.initTableData()
    this.ws = new WebSocket("ws://localhost:9001/websocket/" + this.getUser().username)
